        ...
        with open(str(self.path_to_column_file), encoding=encoding) as file:
            ...
            for line in file:
                ...
                if line.strip().replace("﻿", "") == "":
                    ...
                    # FIXME: TEMPORARY PharmaCoNER dataset error fix!!!
                    if not fields[1].isnumeric():
                        temp_fields = [fields[0], fields[2], None, fields[3]]
                        token = self.create_token(temp_fields, column_name_map)
                        sentence.add_token(token)

                        temp_fields = [fields[1], int(fields[2]) + len(token.text) + 1, None, fields[3]]
                        token = self.create_token(temp_fields, column_name_map)
                        sentence.add_token(token)
                    else:
